...
var (
	ErrOperationTooFrequent   = errors.New("操作过于频繁,请稍后再试！")
	ErrVerifyEmailCaptchaFail = errors.New("校验邮箱验证码失败！")
	ErrServiceClosed          = errors.New("服务未开启！")
	ErrTouristHaveNoRight     = errors.New("游客无权操作！")
	// 对应constant.UserAction
	ErrUnsupportedUserAction = errors.New("暂不支持该操作")
	// 用户
	ErrSameAsTheOldPassword = errors.New("新旧密码相同")
	ErrEmailRegistered      = errors.New("邮箱已注册")
	// 账本
	ErrAccountType = errors.New("账本类型不允许该操作")
	// 交易类型
	ErrCategoryNameEmpty = errors.New("名称不可为空")
	ErrCategorySameName  = errors.New("类型名称相同")
)
